-- Robin - The Star Rail Summeretto (New Version)
local s,id=GetID()
...
function s.initial_effect(c)
    ...
    -- 3 "Halovian"/"Genshin"/"Star Rail" on field + 2 "Halovian"/"Genshin"/"Star Rail" in hand
    ...
    -- Protection 2: Cannot be tributed by opponent
    ...
    -- Protection 4: Cannot be sent to GY by opponent
    ...
    -- Protection 5: Cannot be targeted by card effects
...
function s.altcon(e,c,og,min,max)
    ...
    local mg2=Duel.GetMatchingGroup(s.altmfilter,tp,LOCATION_HAND,0,nil)
...
--------------------------------------------------
-- Trigger Effect: Xyz Summon Success
--------------------------------------------------
...
function s.sumfilter(c,e,tp)
    return c:IsSetCard(0x987) and c:IsRace(RACE_WINGEDBEAST) and c:IsLevel(1) and c:IsAttack(0) 
        and (c:IsAbleToHand() or (e and c:IsCanBeSpecialSummoned(e,0,tp,false,false)))
end
...
function s.sumtg(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,3,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE+LOCATION_REMOVED)
end
...
function s.costfilter2(c)
    return c:IsSetCard(0x987) and c:IsMonster() and c:IsFaceup() and c:IsAbleToGraveAsCost()
end
...
function s.qspfilter(c,e,tp)
    return c:IsSetCard(0x987) and c:IsLevel(1) and c:IsAttack(0) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.qtg(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    local b4=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
        and Duel.IsExistingMatchingCard(s.qspfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil,e,tp)
    ...
    if b1 then ops[off]=aux.Stringid(id,7) opval[off]=0 off=off+1 end
    if b2 then ops[off]=aux.Stringid(id,8) opval[off]=1 off=off+1 end
    if b3 then ops[off]=aux.Stringid(id,9) opval[off]=2 off=off+1 end
    if b4 then ops[off]=aux.Stringid(id,10) opval[off]=3 off=off+1 end
...
function s.qop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if sel==0 then
        ...
        if opt==0 then
            ...
            local fg=Duel.SelectMatchingCard(tp,aux.TRUE,tp,0,LOCATION_ONFIELD,1,1,nil)
    ...
    elseif sel==2 then
        ...
        if tc:IsRelateToEffect(re) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT+REASON_TEMPORARY)>0 then
            local e1=Effect.CreateEffect(e:GetHandler())
